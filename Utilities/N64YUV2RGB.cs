...
namespace AverMediaTestApp.Utilities
{
    public class N64YUV
    {
        ...
        public static void YUY2RGB(byte Y, byte U, byte V, out byte R, out byte G, out byte B)
        {
            R = N64YUV_CLAMP(Y + (int)Math.Round(1.14f * V));
            G = N64YUV_CLAMP(Y - (int)Math.Round(0.395 * U) - (int)Math.Round(0.581 * V));
            B = N64YUV_CLAMP(Y + (int)Math.Round(2.033f * U));
        }

        /** Encodes 2 RGB pixels into 1 UYVY pair */
        public static void RGB2YUV(byte R1, byte G1, byte B1,  byte R2, byte G2, byte B2,  out byte U, out byte Y1, out byte V, out byte Y2)
        {
        ...
        public static byte[] decodeArray_YUY2toRGB(byte[] inputYUY2)
        {
            int inputLength = inputYUY2.Length;
            byte[] outputRGB = new byte[(inputLength * 3) / 2];

            for (int i = 0; i < inputLength / 4; i++)
            {
                byte Y0 = inputYUY2[i * 4 + 0];
                byte U0 = inputYUY2[i * 4 + 1];
                byte Y1 = inputYUY2[i * 4 + 2];
                byte V0 = inputYUY2[i * 4 + 3];

                byte R1, G1, B1, R2, G2, B2;

                YUY2RGB(Y0, U0, V0, out R1, out G1, out B1);
                YUY2RGB(Y1, U0, V0, out R2, out G2, out B2);
